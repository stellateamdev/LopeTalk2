...
public enum HOKColorScheme {
    case lopetalk,hokusai, asagi, matcha, tsubaki, inari, karasu, enshu
    
    func getColors() -> HOKColors {
        switch self {
        case .lopetalk:
            return HOKColors(backGroundColor: UIColor.lopeColor(),
                             buttonColor: UIColorHex(0x3d7d8a), //0x46909e),
...
final public class Hokusai: UIViewController, UIGestureRecognizerDelegate {
    ...
    fileprivate func animationWillStart() {
        ...
        // Menuview
        let size = CGSize(width:menuView.frame.width, height:menuView.frame.height+CGFloat((UIDevice.iPhoneX) ? 34:0))
        menuView.frame = CGRect(origin: CGPoint(x: 0.0, y: self.view.frame.height), size: size) // mark: change because of iPhone X
