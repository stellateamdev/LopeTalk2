...
class CameraView: UIView, UIGestureRecognizerDelegate {
  ...
  func setup() {
    ...
    closeButton.g_pinClose(on: .top) //.top
    closeButton.g_pin(on: .left) //.left
    closeButton.g_pin(size: CGSize(width: 60, height: 60))

    flashButton.g_pinFlash(on: .centerY, view: closeButton)
    flashButton.g_pin(on: .centerX)
    flashButton.g_pin(size: CGSize(width: 60, height: 44))

    rotateButton.g_pinSwitch(on: .top)
    rotateButton.g_pin(on: .right)
    rotateButton.g_pin(size: CGSize(width: 64, height: 64))
