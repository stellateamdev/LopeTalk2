...
extension AVAsset {
  ...
  var g_videoDescription: CMFormatDescription? {
    if let object = tracks(withMediaType: AVMediaType.video).first?.formatDescriptions.first {
  ...
  var g_audioDescription: CMFormatDescription? {
    if let object = tracks(withMediaType: AVMediaType.audio).first?.formatDescriptions.first {
        return (object as! CMFormatDescription)
    }
